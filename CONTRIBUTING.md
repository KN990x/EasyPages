...
- Publishing a **GitHub Release** triggers `.github/workflows/ghcr-publish.yml` and pushes a versioned image to GHCR: the shared pipeline in [KN990x/.github](https://github.com/KN990x/.github) checks that the tag matches `package.json`, that `ci.yml` already passed on the tagged commit, builds the image for amd64 and arm64 from the root [`Dockerfile`](./Dockerfile) and boots both with `.github/scripts/smoke-test.sh`, and only then pushes `X.Y.Z`, `X.Y`, `X` and — for the newest release — `latest`, with provenance and SBOM, and attaches `docker-compose.yml` and `env.example` to the release. The same workflow runs a monthly dry run of `main` (no push). Push the release commit to `main` and let CI finish before tagging it. Keep the `image:` tag in [`docker-compose.yml`](./docker-compose.yml) aligned with the **GitHub Release / GHCR tag** you want users to run by default (the `v2.x.y` tag, not a divergent `package.json` 1.x). When you cut a release, bump `package.json`, the compose pin, and the git tag together.
...
- Session cookies: default `Secure` in production unless you set `SESSION_COOKIE_SECURE` (the shipped `.env.example` sets `false` for HTTP installs). Compose interpolates `.env` into `SESSION_COOKIE_SECURE` / `TRUST_PROXY`; a literal `environment:` value would ignore `.env`.
...
- **Reverse proxy and rate limiting:** the server sets Express `trust proxy` from **`TRUST_PROXY`** (Node default **`1`**, i.e. one trusted hop). Compose defaults to **`false`** for a published HTTP port. In production, terminate TLS, set `TRUST_PROXY=1` in `.env`, and set `X-Forwarded-For` only on a **trusted** proxy so clients cannot spoof IPs and weaken **express-rate-limit**. If the Node process is exposed **directly** to the internet without such a proxy, keep **`TRUST_PROXY=false`** (or `0` / `no`) so limits use the TCP peer address.
